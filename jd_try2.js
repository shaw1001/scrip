/*
update 2021/4/11
京东试用：脚本更新地址 https://raw.githubusercontent.com/ZCY01/daily_scripts/main/jd/jd_try2.js
...
# 京东试用
30 10 * * * https://raw.githubusercontent.com/ZCY01/daily_scripts/main/jd/jd_try2.js, tag=京东试用, img-url=https://raw.githubusercontent.com/ZCY01/img/master/jdtryv1.png, enabled=true
 */
const $ = new Env('京东试用')
...
function requireConfig() {
	return new Promise(resolve => {
		...
		if ($.isNode()) {
			if (process.env.jd_try2_CIDS_KEYS) {
				args.cidsList = process.env.jd_try2_CIDS_KEYS.split('@').filter(key => {
					return Object.keys(cidsMap).includes(key)
				})
			}
			if (process.env.jd_try2_TYPE_KEYS) {
				args.typeList = process.env.jd_try2_TYPE_KEYS.split('@').filter(key => {
					return Object.keys(typeMap).includes(key)
				})
			}
			if (process.env.jd_try2_GOOD_FILTERS) {
				args.goodFilters = process.env.jd_try2_GOOD_FILTERS.split('@')
			}
			if (process.env.jd_try2_MIN_PRICE) {
				args.minPrice = process.env.jd_try2_MIN_PRICE * 1
			}
			if (process.env.jd_try2_PAGE_SIZE) {
				args.pageSize = process.env.jd_try2_PAGE_SIZE * 1
			}
			if (process.env.jd_try2_MAX_SUPPLY_COUNT) {
				args.maxSupplyCount = process.env.jd_try2_MAX_SUPPLY_COUNT * 1
			}
		} else {
			let qxCidsList = []
			let qxTypeList = []
